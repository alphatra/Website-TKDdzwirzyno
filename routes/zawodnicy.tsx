...
import AthletesGrid from "../islands/features/athletes/Grid.tsx";
...
export default define.page(async function Zawodnicy(props) {
  ...
  try {
    ...
    const [athletes, allResults] = await Promise.all([
      // 1. Fetch all athletes
      pb.collection("athletes").getFullList<Athlete>({
        sort: "name",
      }),
      // 2. Fetch all results (Sort manually for safety)
      pb.collection("results").getFullList<Result>({
        // sort: "-competition.date", // PB relation sort can be flaky
        expand: "competition",
      }),
    ]);
    ...
    // Group results and Sort robustly
    // Sort all results first
    allResults.sort((a, b) => {
      // Use numeric sort for dates. Fallback to 0 if invalid.
      // Descending order (newest first).
      const da = a.expand?.competition?.date
        ? new Date(a.expand.competition.date).getTime()
        : (a.expand?.competition?.year
          ? new Date(`${a.expand.competition.year}-01-01`).getTime()
          : 0);

      const db = b.expand?.competition?.date
        ? new Date(b.expand.competition.date).getTime()
        : (b.expand?.competition?.year
          ? new Date(`${b.expand.competition.year}-01-01`).getTime()
          : 0);
  ...
  const activeAthletes = enrichedAthletes.filter((a) =>
  ...
  return (
    <PageShell
      title="Zawodnicy - TKD Dzwirzyno"
      description="Poznaj naszych zawodników. Ich sukcesy, medale i historia."
      menuPages={menuPages}
